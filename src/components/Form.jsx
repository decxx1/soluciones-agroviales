    ...
    return (
        <form onSubmit={handleSubmit} method="post" id="contactForm" className="grid grid-cols-1 gap-8 mx-auto max-w-screen-md sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
                ...
                <textarea id="message" name="mensaje" rows="6" className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg shadow-sm border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Deje su consulta..." required></textarea>
            </div>
            <button disabled={isSending} type="submit" className="py-3 px-5 text-sm font-medium text-center text-gray-700 rounded-lg bg-primary-300 sm:w-fit hover:bg-primary-400 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-300 dark:hover:bg-primary-400 dark:focus:ring-primary-400 dark:text-gray-800">Enviar mensaje</button>
        </form>
    )
}